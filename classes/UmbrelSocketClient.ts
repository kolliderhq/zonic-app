...
import { UMBREL_MESSAGE_TYPES, WS_UMBREL } from 'consts';
...
class UmbrelSocketClient extends EventEmitter {
	...
	public getSynthWallets() {
		this.socketSend(UMBREL_MESSAGE_TYPES.GET_SYNTH_WALLETS)
	}

	public makeConversion(body, cb: (...args: any[]) => void, options) {
		this.socketSend(UMBREL_MESSAGE_TYPES.MAKE_CONVERSION, body, cb, options)
	}
...
const dataRefiner = (type: string, data: any) => {
	switch (type) {
		...
		case WS_UMBREL.MESSAGES.AUTHENTICATION.returnType:
			return data;
		case WS_UMBREL.MESSAGES.GET_SYNTH_WALLETS.returnType:
			return data;
		case "synthWallet":
			return data;
		case "fill":
